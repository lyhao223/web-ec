...
export default function FlyDownSearchBar({SearchInput}: FlyDownSearchBarProps) {
    const [open, setOpen] = useState(false);

    const showFlyout = SearchInput && open;
    
    return(
        <div onClick={()=>setOpen(true)} onMouseLeave={()=>setOpen(false)}>
                ...
                {showFlyout && (
                    ...
                    >
                        ...
                        <div className="absolute top-0 right-4 p-6">
                            <button>
                            <IoMdCloseCircle className="text-white hover:text-red-500 hover:transform hover:ease-out hover:duration-500" size={43}/>
                            </button>
                        </div>
                    </motion.div>
                )}
